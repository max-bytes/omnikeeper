    ...
    public class CLBMonitoring : CLBBase
    {
        ...
        public override async Task<bool> Run(Layer targetLayer, Changeset changeset, CLBErrorHandler errorHandler, NpgsqlTransaction trans, ILogger logger)
        {
            ...
            foreach (var p in allHasMonitoringModuleRelations)
            {
                ...
                var monitoringModuleET = await traitModel.CalculateEffectiveTraitSetForCI(monitoringModuleCI, trans);
            ...
            var naemonInstancesTS = await traitModel.CalculateEffectiveTraitSetsForTraitName("naemon_instance", layerSetAll, trans, DateTimeOffset.Now);
