...
namespace Omnikeeper.Base.Model.TraitBased
{
    ...
    public abstract class SingletonTraitDataConfigBaseModel<T> where T : TraitEntity, new()
    {
            ...
            IEnumerable<string> relationsToRemoveBackward)
        {
            ...
            var relevantRelationsForward = (await relationModel.GetMergedRelations(RelationSelectionFrom.Build(relationsToRemoveForward.ToHashSet(), t.Item1), new LayerSet(writeLayerID), trans, TimeThreshold.BuildLatest(), maskHandlingForRetrieval, GeneratedDataHandlingInclude.Instance)).Select(r => r.Relation);
            var relevantRelationsBackward = (await relationModel.GetMergedRelations(RelationSelectionTo.Build(relationsToRemoveBackward.ToHashSet(), t.Item1), new LayerSet(writeLayerID), trans, TimeThreshold.BuildLatest(), maskHandlingForRetrieval, GeneratedDataHandlingInclude.Instance)).Select(r => r.Relation);
