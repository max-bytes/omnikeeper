...
namespace Omnikeeper.Base.Entity
{
    ...
    {
        private Layer(string id, string description, Color color, AnchorState state, ComputeLayerBrainLink computeLayerBrainLink, OnlineInboundAdapterLink onlineInboundAdapterLink, string[] generators)
        {
            ID = id;
        ...
        [ProtoMember(5)] public readonly ComputeLayerBrainLink ComputeLayerBrainLink; // TODO: why are these links here? isn't a string reference enough?
        ...
        public override int GetHashCode() => HashCode.Combine(ID, Description, State, Color, ComputeLayerBrainLink, OnlineInboundAdapterLink, Generators);
        ...
        public bool Equals(Layer? other) => other != null && ID.Equals(other.ID) && Description.Equals(other.Description)
            && State.Equals(other.State) && ComputeLayerBrainLink.Equals(other.ComputeLayerBrainLink) && OnlineInboundAdapterLink.Equals(other.OnlineInboundAdapterLink) && Enumerable.SequenceEqual(Generators, other.Generators);

        public static Layer Build(string id, string description, Color color, AnchorState state, ComputeLayerBrainLink computeLayerBrain, OnlineInboundAdapterLink onlineInboundAdapter, string[] generators)
        {
            return new Layer(id, description, color, state, computeLayerBrain, onlineInboundAdapter, generators);
        }
    }
}
