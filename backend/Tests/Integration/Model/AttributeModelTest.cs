    ...
    class AttributeModelTest : DBBackedTestBase
    {
        ...
        public async Task TestAddingUpdatingRemovingAndRenewingOfAttributes()
        {
            ...
            using (var trans = ModelContextBuilder.BuildDeferred())
            {
                ...
                var a1 = (await attributeModel.GetMergedAttributes(SpecificCIIDsSelection.Build(ciid1), layerset, trans, TimeThreshold.BuildLatest(), AllAttributeSelection.Instance)).Values.First();
            ...
            using (var trans = ModelContextBuilder.BuildDeferred())
            {
                ...
                var a2 = (await attributeModel.GetMergedAttributes(SpecificCIIDsSelection.Build(ciid1), layerset, trans, TimeThreshold.BuildLatest(), AllAttributeSelection.Instance)).Values;
                Assert.AreEqual(0, a2.Count());

                // compare fetching merged vs non-merged: non-merged returns the removed attribute, merged does not
                var ma3 = await attributeModel.GetMergedAttributes(SpecificCIIDsSelection.Build(ciid1), layerset, trans, TimeThreshold.BuildLatest(), AllAttributeSelection.Instance);
                var a3 = await attributeModel.GetAttributes(SpecificCIIDsSelection.Build(ciid1), layerset.LayerIDs, true, trans, TimeThreshold.BuildLatest(), AllAttributeSelection.Instance);
            ...
            using (var trans = ModelContextBuilder.BuildDeferred())
            {
                ...
                var a4 = (await attributeModel.GetMergedAttributes(SpecificCIIDsSelection.Build(ciid1), layerset, trans, TimeThreshold.BuildLatest(), AllAttributeSelection.Instance)).Values.First();
        ...
        public async Task TestAttributeValueMultiplicities()
        {
            ...
            using (var trans = ModelContextBuilder.BuildDeferred())
            {
                ...
                var a1 = (await attributeModel.GetMergedAttributes(SpecificCIIDsSelection.Build(ciid1), layerset1, trans, TimeThreshold.BuildLatest(), AllAttributeSelection.Instance)).Values.First();
                Assert.AreEqual(1, a1.Count());
        ...
        public async Task TestGetAttributesWithNameRegex()
        {
            ...
            var a1 = (await attributeModel.GetAttributes(new AllCIIDsSelection(), new string[] { layer1.ID }, returnRemoved: false, trans, TimeThreshold.BuildLatest(), new RegexAttributeSelection("^a"))).SelectMany(t => t.Values.SelectMany(t => t.Values));
            Assert.AreEqual(2, a1.Count());

            var a2 = (await attributeModel.GetAttributes(new AllCIIDsSelection(), new string[] { layer1.ID }, returnRemoved: false, trans, TimeThreshold.BuildLatest(), new RegexAttributeSelection("^a2$"))).SelectMany(t => t.Values.SelectMany(t => t.Values));
            Assert.AreEqual(1, a2.Count());

            var a3 = (await attributeModel.GetAttributes(new AllCIIDsSelection(), new string[] { layer2.ID }, returnRemoved: false, trans, TimeThreshold.BuildLatest(), new RegexAttributeSelection("3$"))).SelectMany(t => t.Values.SelectMany(t => t.Values));
            Assert.AreEqual(1, a3.Count());

            var a4 = (await attributeModel.GetAttributes(new AllCIIDsSelection(), new string[] { layer1.ID }, returnRemoved: false, trans, TimeThreshold.BuildLatest(), new RegexAttributeSelection("^3"))).SelectMany(t => t.Values.SelectMany(t => t.Values));
            Assert.AreEqual(0, a4.Count());

            var a5 = (await attributeModel.GetAttributes(SpecificCIIDsSelection.Build(ciid2), new string[] { layer2.ID }, returnRemoved: false, trans, TimeThreshold.BuildLatest(), new RegexAttributeSelection("^a1$"))).SelectMany(t => t.Values.SelectMany(t => t.Values));
        ...
        public async Task TestBulkReplace()
        {
            ...
            var a1 = (await attributeModel.GetAttributes(new AllCIIDsSelection(), new string[] { layer1.ID }, returnRemoved: false, trans3, TimeThreshold.BuildLatest(), new RegexAttributeSelection("^prefix1"))).SelectMany(t => t.Values.SelectMany(t => t.Values));
            Assert.AreEqual(3, a1.Count());
            Assert.AreEqual(1, a1.Where(a => a.Name == "prefix1.a2").Count());
            var a2 = (await attributeModel.GetAttributes(new AllCIIDsSelection(), new string[] { layer1.ID }, returnRemoved: false, trans3, TimeThreshold.BuildLatest(), new RegexAttributeSelection("^prefix2"))).SelectMany(t => t.Values.SelectMany(t => t.Values));
            Assert.AreEqual(1, a2.Count());
        }
        ...
        public async Task TestLayerSets()
        {
            ...
            var a1 = (await attributeModel.GetMergedAttributes(SpecificCIIDsSelection.Build(ciid1), layerset1, trans, TimeThreshold.BuildLatest(), AllAttributeSelection.Instance)).SelectMany(t => t.Value.Values);
            Assert.AreEqual(1, a1.Count());
            Assert.AreEqual(new AttributeScalarValueText("textL1"), a1.First().Attribute.Value);

            var a2 = (await attributeModel.GetMergedAttributes(SpecificCIIDsSelection.Build(ciid1), layerset2, trans, TimeThreshold.BuildLatest(), AllAttributeSelection.Instance)).SelectMany(t => t.Value.Values);
            Assert.AreEqual(1, a2.Count());
            Assert.AreEqual(new AttributeScalarValueText("textL2"), a2.First().Attribute.Value);

            var a3 = (await attributeModel.GetMergedAttributes(SpecificCIIDsSelection.Build(ciid1), layerset3, trans, TimeThreshold.BuildLatest(), AllAttributeSelection.Instance)).SelectMany(t => t.Value.Values);
            Assert.AreEqual(1, a3.Count());
            Assert.AreEqual(new AttributeScalarValueText("textL1"), a3.First().Attribute.Value);

            var a4 = (await attributeModel.GetMergedAttributes(SpecificCIIDsSelection.Build(ciid1), layerset4, trans, TimeThreshold.BuildLatest(), AllAttributeSelection.Instance)).SelectMany(t => t.Value.Values);
            Assert.AreEqual(1, a4.Count());
        ...
        public async Task TestRemoveShowsLayerBelow()
        {
            ...
            var a1 = (await attributeModel.GetMergedAttributes(SpecificCIIDsSelection.Build(ciid1), layerset1, transI, TimeThreshold.BuildLatest(), AllAttributeSelection.Instance)).Values.First();
