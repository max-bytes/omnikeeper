...
{
    class GeneratorModelTest : GenericTraitEntityModelTestBase
    {
        [Test]
        public void TestTraitGeneration()
        {
            var et = TraitBuilderFromClass.Class2RecursiveTrait<GeneratorV1>();

            et.Should().BeEquivalentTo(
                new RecursiveTrait("__meta.config.generator", new TraitOriginV1(TraitOriginType.Core),
                    new List<TraitAttribute>() {
                        new TraitAttribute("id", CIAttributeTemplate.BuildFromParams("generator.id", AttributeValueType.Text, false, CIAttributeValueConstraintTextLength.Build(1, null), new CIAttributeValueConstraintTextRegex(IDValidations.GeneratorIDRegexString, IDValidations.GeneratorIDRegexOptions))),
