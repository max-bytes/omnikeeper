    ...
    class BaseRelationRevisionistModelTest : DIServicedTestBase
    {
        ...
        public async Task TestArchiveOutdatedRelationsOlderThan()
        {
            ...
            using (var trans = ModelContextBuilder.BuildDeferred())
            {
                ...
                await GetService<IRelationModel>().InsertRelation(ciid1, ciid2, "p1", layerID1, changeset, new DataOriginV1(DataOriginType.Manual), trans);
                await GetService<IRelationModel>().InsertRelation(ciid1, ciid2, "p2", layerID1, changeset, new DataOriginV1(DataOriginType.Manual), trans);
                trans.Commit();
            }
