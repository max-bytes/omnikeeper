...
namespace Omnikeeper.GraphQL
{
    public partial class GraphQLQueryRoot : ObjectGraphType
    {
        public GraphQLQueryRoot()
        {
            ...
            //         var (_, predicate) = await predicateModel.TryToGetPredicate(predicateID, userContext.TimeThreshold, userContext.Transaction);


            //         IEnumerable<CompactCI> cis;
            //         // predicate ist not defined or has no target constraints -> makes it easy, return ALL CIs
            //         if (predicate == null || (forward && !predicate.Constraints.HasPreferredTraitsTo) || (!forward && !predicate.Constraints.HasPreferredTraitsFrom))
