...
namespace Omnikeeper.GridView.Helper
{
    public class AttributeResolver
    {
        ...
        public async Task PrefetchRelatedCIsAndLookups(GridViewConfiguration config, ISet<Guid> baseCIIDs, IRelationModel relationModel, ICIModel ciModel, IModelContext trans, TimeThreshold atTime)
        {
            ...
            var relevantOutgoingPredicates = pathsRequiringRelations.Where(path => path.direction == ">").Select(t => t.predicateID).ToHashSet();
            var relevantIncomingPredicates = pathsRequiringRelations.Where(path => path.direction == "<").Select(t => t.predicateID).ToHashSet();

            ISet<Guid> outgoingRelatedCIIDs = new HashSet<Guid>();
            if (relevantOutgoingPredicates.Count() > 0)
            {
                // TODO: masking
                var outgoingRelations = await relationModel.GetMergedRelations(RelationSelectionFrom.Build(relevantOutgoingPredicates, baseCIIDs), new LayerSet(config.ReadLayerset), trans, atTime, MaskHandlingForRetrievalGetMasks.Instance, GeneratedDataHandlingInclude.Instance);
            ...
            if (relevantIncomingPredicates.Count() > 0)
            {
                // TODO: masking
                var incomingRelations = await relationModel.GetMergedRelations(RelationSelectionTo.Build(relevantIncomingPredicates, baseCIIDs), new LayerSet(config.ReadLayerset), trans, atTime, MaskHandlingForRetrievalGetMasks.Instance, GeneratedDataHandlingInclude.Instance);
