            ...
            public async Task<Exception?> Handle(Command request, CancellationToken cancellationToken)
            {
                ...
                try
                {
                    var isSuccess = await gridViewContextModel.TryToDelete(request.ID,
                        ...
                        new Base.Entity.DataOrigin.DataOriginV1(Base.Entity.DataOrigin.DataOriginType.Manual), changesetProxy, trans, MaskHandlingForRemovalApplyNoMask.Instance, OtherLayersValueHandlingForceWrite.Instance);
