...
{
    public class ValidationModel : TraitDataConfigBaseModel<Base.Validation.Validation, string>, IValidationModel
    {
        ...
        public async Task<IDictionary<string, Base.Validation.Validation>> GetValidations(LayerSet layerSet, IModelContext trans, TimeThreshold timeThreshold)
        {
            return await GetAll(layerSet, trans, timeThreshold);
        }

        protected override (Base.Validation.Validation, string) EffectiveTrait2DC(EffectiveTrait et, MergedCI ci)
        {
            ...
            return (new Base.Validation.Validation(id, ruleName, ruleConfig), id);
        }
        ...
        public async Task<(Base.Validation.Validation validationIssue, bool changed)> InsertOrUpdate(string id, string ruleName, JObject ruleConfig, LayerSet layerSet, string writeLayerID, DataOriginV1 dataOrigin, IChangesetProxy changesetProxy, IModelContext trans)
        {
