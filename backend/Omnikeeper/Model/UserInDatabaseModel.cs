...
namespace Omnikeeper.Model
{
    public class UserInDatabaseModel : IUserInDatabaseModel
    {
        ...
        public async Task<UserInDatabase> UpsertUser(string username, string displayName, Guid uuid, UserType type, NpgsqlTransaction trans)
        {
            ...
            using var command = new NpgsqlCommand(@"INSERT INTO ""user"" (keycloak_id, timestamp, type, username, displayName) VALUES (@uuid, @timestamp, @type, @username, @displayName) returning id, timestamp", conn, trans);
            ...
            return UserInDatabase.Build(id, uuid, username, displayName, type, timestamp);
        }

        public async Task<UserInDatabase> GetUser(long id, NpgsqlTransaction trans)
        {
            using var command = new NpgsqlCommand(@"SELECT keycloak_id, username, displayName, type, timestamp FROM ""user"" WHERE id = @id ORDER BY timestamp DESC LIMIT 1", conn, trans);
            ...
            return UserInDatabase.Build(id, uuid, username, displayName, usertype, timestamp);
        }
