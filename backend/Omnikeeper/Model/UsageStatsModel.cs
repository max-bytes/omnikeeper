...
namespace Omnikeeper.Model
{
    public class UsageStatsModel : IUsageStatsModel
    {
        ...
        {
            var query = @"SELECT element_type, element_name, username, timestamp FROM public.usage_stats where timestamp >= @from AND timestamp <= @to";
            ...
            while (dr.Read())
            {
                ...
                var timestamp = dr.GetDateTime(3);

                var e = new UsageStatElement(type, name, username, timestamp);
                ret.Add(e);
            }
            return ret;
        }
