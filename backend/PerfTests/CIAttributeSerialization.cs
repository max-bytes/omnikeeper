...
namespace PerfTests
{
  ...
  ""bla"": {
    ""blub"": true
            ...
            aScalarText = new CIAttribute(Guid.NewGuid(), "test", Guid.NewGuid(), new AttributeScalarValueText("foo"), AttributeState.New, Guid.NewGuid(), new DataOriginV1(DataOriginType.Manual));
            aScalarYAML = new CIAttribute(Guid.NewGuid(), "test", Guid.NewGuid(), AttributeScalarValueYAML.BuildFromString(testYAML), AttributeState.New, Guid.NewGuid(), new DataOriginV1(DataOriginType.Manual));
            aScalarJSON = new CIAttribute(Guid.NewGuid(), "test", Guid.NewGuid(), AttributeScalarValueYAML.BuildFromString(testJSON), AttributeState.New, Guid.NewGuid(), new DataOriginV1(DataOriginType.Manual));
